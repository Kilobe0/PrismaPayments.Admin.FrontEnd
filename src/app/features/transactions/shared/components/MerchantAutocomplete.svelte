<script lang="ts">
  ...
  import type { PaginatedMerchants, MerchantListItem } from '$appmod/features/merchants/domain/entities/Merchant';
  ...
  async function fetchMerchants(term: string) {
    ...
    try {
      const res = await apiClient.get<PaginatedMerchants>(
        `${API_PATHS.ADMIN_MERCHANTS}?search=${encodeURIComponent(term)}&limit=10`
      );
...
<div style="position: relative; display: flex; flex-direction: column; gap: 4px; min-width: 200px;">
  <label style="font-family: 'Outfit', sans-serif; font-size: 0.75rem; font-weight: 500; color: var(--color-foreground-secondary, #9090A8); text-transform: uppercase; letter-spacing: 0.08em;">
    Merchant
  </label>
